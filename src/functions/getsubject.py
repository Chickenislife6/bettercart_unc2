...
import threading
from datareq.datatypes.Class import Class
...
def get_cart(instance: SessionWrapper, subject, var: bool, attribute: str):
    ...
    futures = {p.submit(run, instance, i): i for i in range(num_classes)}
...
def run(instance, i):
    ...
    # print(process.memory_info().rss, class_num)
    gc.collect()
    # Class(name, open, reserved, waitlist, time, prof, description, attribute)
    return (class_num, name, open, reserved, waitlist, time, prof, description, attribute)